...
void drawPlayer() {
 shadowOAM[0].attr0 = player.row | (0<<13) | (1<<14);
...
void updatePlayer() {
 if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<4))) && player.col + player.width < 238) {
  player.col++;
 }
...
void updateEnemy(ENEMY* e) {
 ...
 for (int i = 0; i < 25; i++) {
  ...
  if (enemies[i].col < 1 || enemies[i].col + enemies[i].width > 240 - 1) {
   e->cdel *= -1;
   e->row += e->rdel;
  }
 }
